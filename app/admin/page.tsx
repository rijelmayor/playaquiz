...
import { type AdminJobOrderDetailRow } from "@/components/admin/JobOrderDetailManager";
import { type AdminPaymentJob } from "@/components/admin/AdminPaymentManager";
import { type CompletionRow } from "@/components/admin/CompletionAcknowledgment";
import { type MaterialRow } from "@/components/admin/AdminMaterialsEditor";
...
export default async function AdminPortal() {
  ...
  const queryNames = [
    ...
    "job_orders", "job_attachments", "fabricators", "quotation_settings", "job_acknowledgments", "audit_logs"
  ];
  results.forEach((r, i) => {
    if (!r.error) return;
  ...
  const auditTrailRows = (auditRows ?? []).map((r: any) => ({
    audit_id: r.audit_id,
    action: r.action,
  ...
  const [adminMaterials, adminQc, adminInstallations, adminRequests, adminHistory] = await Promise.all([
    adminJobOrderIds.length
      ? supabase
          .from("job_order_materials")
          .select(
            "material_id,job_order_id,material_name,specification,unit,estimated_qty,estimated_unit_cost,actual_qty,actual_unit_cost,status"
          )
          .in("job_order_id", adminJobOrderIds)
          .order("created_at", { ascending: true })
      : Promise.resolve({ data: [], error: null } as any),
    adminJobOrderIds.length
      ? supabase
          .from("job_order_qc_checks")
          .select("qc_id,job_order_id,result,rework_required,inspected_at")
          .in("job_order_id", adminJobOrderIds)
    ...
    adminJobOrderIds.length
      ? supabase
          ...
          .select(
            "installation_id,job_order_id,status,verified,scheduled_date,updated_at,created_at"
          )
          .in("job_order_id", adminJobOrderIds)
    ...
    adminJobOrderIds.length
      ? supabase
          .from("job_order_material_requests")
          .select("request_id,job_order_id,status")
          .in("job_order_id", adminJobOrderIds)
      : Promise.resolve({ data: [], error: null } as any),
    adminJobOrderIds.length
      ? supabase
          .from("job_order_stage_history")
          .select("history_id,job_order_id,from_stage,to_stage,changed_at,note")
          .in("job_order_id", adminJobOrderIds)
          .order("changed_at", { ascending: false })
      : Promise.resolve({ data: [], error: null } as any)
  ]);
  for (const [name, result] of [
    ["job_order_materials", adminMaterials],
    ["job_order_qc_checks", adminQc],
    ["job_order_installations", adminInstallations],
    ["job_order_material_requests", adminRequests],
    ["job_order_stage_history", adminHistory]
  ] as const) {
    if (result.error) throw new Error(`Query "${name}" failed: ${result.error.message}`);
  }

  const materialsByOrder: Record<string, MaterialRow[]> = {};
  for (const m of adminMaterials.data ?? []) {
    const list = materialsByOrder[m.job_order_id] ?? [];
    list.push({
    ...
    materialsByOrder[m.job_order_id] = list;
  }
  ...
  const completionRows: CompletionRow[] = (acknowledgments ?? [])
    .filter((a: any) =>
      (jobs ?? []).some(
        (j: any) =>
          j.job_id === a.job_id && ["installed", "paid", "closed"].includes(j.status)
      )
    )
    .map((a: any) => {
      ...
      return {
        ...
        job_id: a.job_id,
        client_name:
          (Array.isArray(job?.clients) ? job.clients[0] : job?.clients)?.name ??
          "Unknown",
        display_job_id:
          quote?.project_job_id ?? `JOB-${a.job_id.slice(0, 8).toUpperCase()}`,
        status: a.status,
        customer_name: a.customer_name,
        authorized_representative: a.authorized_representative,
        signature_name: a.signature_name,
        remarks: a.remarks,
        installation_checked: Boolean(a.installation_checked),
        project_received: Boolean(a.project_received)
      };
    });
  ...
  const adminJobs: AdminJobRow[] = (jobs ?? []).map((job: any) => {
    ...
    const paymentTerms: PaymentTerms =
      job.payment_terms ?? quote?.payment_terms ?? "50_50";

    return {
      job_id: job.job_id,
      display_job_id:
        quote?.project_job_id ?? `JOB-${job.job_id.slice(0, 8).toUpperCase()}`,
      job_name: job.job_name,
      client_name:
        (Array.isArray(job.clients) ? job.clients[0] : job.clients)?.name ??
        "Unknown",
      contact:
        (Array.isArray(job.clients) ? job.clients[0] : job.clients)?.contact ??
        null,
      email:
        (Array.isArray(job.clients) ? job.clients[0] : job.clients)?.email ??
        null,
      location:
        (Array.isArray(job.clients) ? job.clients[0] : job.clients)?.location ??
        null,
      ...
      quotation_count: quotationRows.filter((q) => q.job_id === job.job_id)
        .length,
      ...
      reference_photos: attachmentsWithUrls.filter(
        (a) => a.job_id === job.job_id && a.category === "reference"
      ),
      transaction_photos: attachmentsWithUrls.filter(
        (a) => a.job_id === job.job_id && a.category === "transaction"
      ),
      site_visit_photos: attachmentsWithUrls.filter(
        (a) => a.job_id === job.job_id && a.category === "site_visit"
      )
    };
  });
  ...
  const adminPaymentJobs: AdminPaymentJob[] = adminJobs.map((job) => ({
    ...
    quote_total:
      job.latest_quotation_total ?? job.quoted_value ?? job.final_value ?? 0,
  ...
  const pendingQuoteRows = adminJobs
    ...
    .map((job) => ({
      job_id: job.job_id,
      client_name: job.client_name,
      quoted_value: job.latest_quotation_total ?? job.quoted_value ?? 0,
      quotation_id: job.latest_quotation_id,
      version:
        quotationRows.find((q: any) => q.quotation_id === job.latest_quotation_id)
          ?.version ?? null
    }));
  ...
  const pendingDesignRows = designRows
    ...
    .map((d: any) => ({
      ...
      job_id: d.job_id,
      client_name:
        adminJobs.find((j) => j.job_id === d.job_id)?.client_name ?? "Unknown",
  ...
  const jobOrderCandidateRows = adminJobs
    ...
    .map((job) => ({
      ...
      down_payment_received:
        job.payment_terms !== "50_50" || job.deposit_received > 0
    }));

  const latestQcByOrder = new Map<string, any>();
  for (const q of adminQc.data ?? [])
    if (!latestQcByOrder.has(q.job_order_id))
      latestQcByOrder.set(q.job_order_id, q);
  const latestInstallationByOrder = new Map<string, any>();
  for (const i of adminInstallations.data ?? [])
    if (!latestInstallationByOrder.has(i.job_order_id))
      latestInstallationByOrder.set(i.job_order_id, i);
  const materialCountsByOrder = new Map<
    string,
    { total: number; completed: number; shortages: number }
  >();
  for (const m of adminMaterials.data ?? []) {
    const current = materialCountsByOrder.get(m.job_order_id) ?? {
      total: 0,
      completed: 0,
      shortages: 0
    };
    current.total += 1;
    if (
      ["used", "available"].includes(m.status) ||
      (m.actual_qty != null && m.actual_qty > 0)
    )
      current.completed += 1;
  ...
  for (const r of adminRequests.data ?? [])
    if (r.status === "pending")
      pendingRequestsByOrder.set(
        r.job_order_id,
        (pendingRequestsByOrder.get(r.job_order_id) ?? 0) + 1
      );

  const fabricatorNames = new Map(
    (fabricators ?? []).map((f: any) => [f.user_id, f.name])
  );
  const jobOrderDetailRows: AdminJobOrderDetailRow[] = orderRows.map(
    (order: any) => {
      ...
      return {
        ...
        job_value: Number(job?.final_value ?? job?.quoted_value ?? 0),
        production_stage: order.production_stage ?? "materials",
        deadline: order.deadline ?? null,
        material_summary: materialCountsByOrder.get(order.job_order_id) ?? {
          total: 0,
          completed: 0,
          shortages: 0
        },
        pending_material_requests:
          pendingRequestsByOrder.get(order.job_order_id) ?? 0,
        latest_qc: latestQcByOrder.get(order.job_order_id) ?? null,
        latest_installation:
          latestInstallationByOrder.get(order.job_order_id) ?? null,
        stage_history: (adminHistory.data ?? []).filter(
          (h: any) => h.job_order_id === order.job_order_id
        ),
        ...
        approved_design: attachmentsWithUrls.filter(
          (a) =>
            a.job_order_id === order.job_order_id &&
            a.category === "approved_design"
        ),
        order_reference: attachmentsWithUrls.filter(
          (a) =>
            a.job_order_id === order.job_order_id &&
            a.category === "order_reference"
        )
      };
    }
  );
  ...
  return (
    ...
    >
      <AdminWorkspace
        jobs={adminJobs}
        paymentJobs={adminPaymentJobs}
        preQuoteRows={preQuoteRows}
        pendingQuoteRows={pendingQuoteRows}
        pendingDesignRows={pendingDesignRows}
        jobOrderCandidateRows={jobOrderCandidateRows}
        jobOrderDetailRows={jobOrderDetailRows}
