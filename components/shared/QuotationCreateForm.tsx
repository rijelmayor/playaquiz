...
interface ItemRow { id: string; description: string; quantity: string; unit: string; unit_price: string; }
...
function emptyRow(): ItemRow { return { id: crypto.randomUUID(), description: "", quantity: "1", unit: "unit", unit_price: "" }; }
...
export function QuotationCreateForm({ rows, settings, createdBy }: { rows: JobNeedingQuote[]; settings: QuotationSettings | null; createdBy: string; }) {
  ...
  async function create(jobId: string, version: number) {
    const cleanItems = items.filter((it) => it.description.trim()).map((it) => ({ id: it.id, description: it.description.trim(), quantity: Number(it.quantity) || 0, unit: it.unit.trim() || "unit", unit_price: Number(it.unit_price) || 0 }));
      ...
      {rows.map((row) => (
        <div key={row.job_id} className="border-b border-gray-100 px-4 py-3 last:border-0">
          ...
          {openId === row.job_id && <div className="mt-3 space-y-3 rounded-lg border border-gray-200 bg-gray-50 p-3">
            ...
            <div><label className="mb-1 block text-[11px] font-medium text-gray-500">Items</label><div className="overflow-x-auto"><div className="min-w-[720px]"><div className="grid grid-cols-[1fr_64px_72px_110px_130px_36px] gap-1.5 px-1 pb-1 text-[10px] font-semibold uppercase tracking-wide text-gray-400"><span>Description</span><span>Qty</span><span>Unit</span><span>Unit Price</span><span>Line Total</span><span></span></div><div className="space-y-1.5">{items.map((it) => { const lineTotal = (Number(it.quantity) || 0) * (Number(it.unit_price) || 0); return <div key={it.id} className="grid grid-cols-[1fr_64px_72px_110px_130px_36px] items-center gap-1.5"><input placeholder="Description" value={it.description} onChange={(e) => updateItem(it.id, { description: e.target.value })} className={inputClass} /><input type="number" min="0" placeholder="Qty" value={it.quantity} onChange={(e) => updateItem(it.id, { quantity: e.target.value })} className={inputClass} /><input value={it.unit} onChange={(e) => updateItem(it.id, { unit: e.target.value })} className={inputClass} placeholder="pcs" /><input type="number" min="0" placeholder="Price" value={it.unit_price} onChange={(e) => updateItem(it.id, { unit_price: e.target.value })} className={inputClass} /><div className="rounded-md border border-gray-200 bg-gray-50 px-2.5 py-2 text-right text-xs font-semibold text-gray-900">₱{lineTotal.toLocaleString("en-PH", { minimumFractionDigits: 2 })}</div><button type="button" onClick={() => removeItem(it.id)} className="rounded-md border border-gray-300 px-2 py-2 text-xs text-gray-500">✕</button></div>; })}</div></div></div><button type="button" onClick={addItem} className="mt-1.5 text-xs font-medium text-[#087eb9]">+ Add item</button></div>
