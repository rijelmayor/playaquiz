...
export function CommissionSettingsForm({ settings, accountingUserId }: { settings: CommissionSettingsRow | null; accountingUserId?: string }) {
  ...
  async function save() {
    ...
    const { error: saveError } = await supabase.from("commission_settings").upsert({
      ...
      updated_by: accountingUserId || null,
  ...
  return (
    <div className="mb-6 rounded-xl border border-gray-200 bg-white shadow-sm">
      <div className="border-b border-gray-200 px-4 py-3">
        ...
        <p className="text-xs text-gray-500">Accounting controls the default used when a new job commission is created. Existing job commissions keep their saved terms.</p>
      </div>
